...
;; recenter の位置
;; (setq recenter-positions '(0.4 top bottom))
...
;; describe-key で何のコマンドに使われているか調べられる
;; 設定する時にキーの表記方法がわからなかったら上記で表示されたキーを(kbd " ")の中に入れれば良い


;; 基本キーバインド
(keyboard-translate ?\C-h ?\C-?)	;; エコー領域で使うためにキーごと置き換える
(global-set-key "\C-h" nil)
(global-unset-key "\C-u")		;; C-u をPrefix Commandとして使う
(global-set-key "\C-x\C-q" 'kill-this-buffer)
(global-set-key "\C-m" 'newline-and-indent)
;; (global-set-key "\C-z" 'toggle-truncate-lines) ;; 行の折り返し
;; (global-set-key "\C-z" (lambda ()(interactive) (insert-tab))) ;; M-i で代用可能
(global-set-key "\C-t" 'yank-pop)	;; yank の後に打てばkill ring を遡る
(global-unset-key "\C-x\C-u")
(global-set-key "\C-x\C-u\C-o" 'isearch-forward)
...
;; テキストモードで選択範囲の文字数を数える ( ES用 )
(define-key text-mode-map (kbd "C-M-c") 'count-words-region)
(define-key text-mode-map (kbd "C-r") (lambda ()(interactive)
                                        (message (concat "the number of characters in this line : "
                                                         (number-to-string
...
;; 上記の強制的にキーバインドを割り当てる設定は汎用性が高く非常に便利
;; 参考 : http://d.hatena.ne.jp/rubikitch/20101126/keymap)
...
(defun open-line-next-indent ()
  ...
  (end-of-line 0)
  (newline-and-indent))
(global-set-key (kbd "C-M-o") 'open-line-next-indent)
...
;; key-chord で同時押しできなかった場合の関数
(defun recover-key-chord (a b)
  "カーソル前の失敗した文字を消す"
  (let ((flag t))
    (while (eq flag t)
      (setf flag nil)
      (if (eq (preceding-char) a)
	  (when (eq (char-before (1- (point))) b)
	    (undo-tree-undo)
	    (setf flag t))
	(if (eq (preceding-char) b)
	    (when (eq (char-before (1- (point))) a)
	      (undo-tree-undo)
	      (setf flag t)))))))
...
;; 適応インデント
(defun adaptive-indent ()
  "範囲選択時も対応"
  "マクロ定義をまとめてインデントするのに便利"
  (interactive)
  (if (not (region-active-p))
      (tab-to-tab-stop)
    (let ((line (count-lines (region-beginning) (region-end)))
          (beg)
          (cnt 0))
      (goto-char (region-beginning))
      (setf beg (line-beginning-position))
      (while (< cnt line)
        ...
        (beginning-of-line 2)
        (incf cnt))
      ;; (set-mark (point)) ;;何故か効かない
      (goto-char beg)
      )))
(global-set-key "\C-z" 'adaptive-indent)
...
;; 画面を眺めることに特化したモード
    ...
    ;; 右手用
    ("n" . ,(lambda ()(interactive) (paragraph-scroll-up 4)))
    ("p" . ,(lambda ()(interactive) (paragraph-scroll-down 4)))
    ("j" . ,(lambda ()(interactive) (scroll-up 10)))
    ("k" . ,(lambda ()(interactive) (scroll-down 10)))
    ("u" . ,(lambda ()(interactive) (scroll-up 1)))
    ("i" . ,(lambda ()(interactive) (scroll-down 1)))
    ("h" . other-window)
    ("l" . recenter-top-bottom)
    ;; 左手用（左右対称）
    ("v" . ,(lambda ()(interactive) (paragraph-scroll-up 4)))
    ("q" . ,(lambda ()(interactive) (paragraph-scroll-down 4)))
    ("f" . ,(lambda ()(interactive) (scroll-up 10)))
    ("d" . ,(lambda ()(interactive) (scroll-down 10)))
    ("r" . ,(lambda ()(interactive) (scroll-up 1)))
    ("e" . ,(lambda ()(interactive) (scroll-down 1)))
    ("g" . other-window)
    ("s" . recenter-top-bottom)
...
;; (global-set-key (kbd "C-@") 'er/expand-region)   ;; 範囲を拡げる
(global-set-key (kbd "<escape>") 'er/expand-region)   ;; Karabiner用
;; (global-set-key (kbd "C-:") 'er/contract-region) ;; 範囲を狭める
(global-set-key (kbd "C-]") 'er/contract-region) ;; 範囲を狭める
...
;; tree起動
(global-set-key (kbd "C-u C-u") 'undo-tree-visualize)

;; undo および redo
(global-set-key (kbd "C-j") 'undo-tree-undo)
(global-set-key (kbd "C-M-j") 'undo-tree-redo)
...
;; 保存させないファイルの指定
;; (setq undohist-ignored-files '("/elpa/" "/\\.git/" "snippets/"))
...
;; 許容する打鍵間隔
(setq key-chord-two-keys-delay 0.03)	;; キーが2種類の場合
(setq key-chord-one-key-delay 0.12)	;; キーが1種類の場合


;; キーバインド
;; (key-chord-define-global "vn" (lambda ()(interactive) (recover-key-chord ?v ?n)(helm-occur)))
(key-chord-define-global "vn" (lambda ()(interactive) (recover-key-chord ?v ?n)(helm-swoop)))
(key-chord-define-global "sd" (lambda ()(interactive) (recover-key-chord ?s ?d)(save-buffer)))
(key-chord-define-global "zs" (lambda ()(interactive) (recover-key-chord ?z ?s)(iedit-mode)))
(key-chord-define-global "gh" (lambda ()(interactive) (recover-key-chord ?g ?h)(iedit-mode)))
;; (key-chord-define-global "34" (lambda ()(interactive) (recover-key-chord ?3 ?4)(anzu-query-replace-at-cursor)))
(key-chord-define-global "45" 'anzu-query-replace)

;; ウィンドウ操作系
(key-chord-define-global "12" (lambda ()(interactive) (recover-key-chord ?1 ?2)(delete-other-windows)))
(key-chord-define-global "22" 'split-window-below)
(key-chord-define-global "13" (lambda ()(interactive) (recover-key-chord ?1 ?3)(split-window-below)))
(key-chord-define-global "23" (lambda ()(interactive) (recover-key-chord ?2 ?3)(split-window-right)))
(key-chord-define-global "33" (lambda ()(interactive) (recover-key-chord ?3 ?3)(split-window-horizontally-n 3)))
(key-chord-define-global "14" (lambda ()(interactive) (recover-key-chord ?1 ?4)(split-window-right)
				(split-window-below)(other-window 2)(split-window-below)(other-window 2)))
(key-chord-define-global "90" (lambda ()(interactive) (recover-key-chord ?9 ?0)(delete-window)))
(key-chord-define-global "fj" (lambda ()(interactive) (recover-key-chord ?f ?j)(other-window 1)))
(key-chord-define-global "dk" (lambda ()(interactive) (recover-key-chord ?d ?k)(other-window -1)))
(key-chord-define-global "qw" (lambda ()(interactive) (recover-key-chord ?q ?w)(kill-this-buffer)))

;; view-mode
(key-chord-define-global "jk" (lambda ()(interactive) (recover-key-chord ?j ?k)'(view-mode)(toggle-read-only)))
(key-chord-define-global "df" (lambda ()(interactive) (recover-key-chord ?d ?f)'(view-mode)(toggle-read-only)))

;; "kl" : 各モードの save-buffer 系
(key-chord-define-global "kl" (lambda ()(interactive) (recover-key-chord ?k ?l)(save-buffer)))
(key-chord-define emacs-lisp-mode-map "kl" (lambda ()(interactive) (recover-key-chord ?k ?l)(bookmark-save)(save-buffer)(eval-buffer)))
;; (eval-after-load 'yatex '(key-chord-define YaTeX-mode-map "kl" (lambda ()(interactive)(recover-key-chord ?k ?l)(compile-preview))))
(with-eval-after-load 'yatex
  (key-chord-define YaTeX-mode-map "kl" (lambda ()(interactive)(recover-key-chord ?k ?l)(compile-preview)))
  ;; (key-chord-define YaTeX-mode-map "l;" (lambda ()(interactive)(recover-key-chord ?l ?\;)(YaTeX-make-begin-end))))
  (key-chord-define YaTeX-mode-map "l;" 'YaTeX-make-begin-end))


;; 編集系
;; (key-chord-define-global "p@" 'describe-key) ;; JIS
(key-chord-define-global "p[" 'describe-key)
;; (key-chord-define-global ";:" 'describe-mode) ;; JIS
(key-chord-define-global "[]" 'describe-mode)
(key-chord-define-global ";'" 'describe-function)
(key-chord-define-global "./" 'helm-apropos)
